 ...
 * 
 */
package br.com.mackenzie.pos.songpop.util;
...
public class PaginationTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		int actualPage = ((pagination.getStartPage() == 0) ? 1 : pagination
				.getStartPage());
		...
		if (pagination.getPagesTotal() > 1) {
			...
			if (pagination.getTotal() >= pagination.getSize()) {
				...
				if (pagination.getStartPage() > 1) {
					...
					if (pagination.getStart() > (pagination.getStep() / 2) + 1
							&& pagination.getPagesTotal() > pagination
									.getStep()) {
				...
				// page numbers
				for (Iterator<Integer> iterator = getPageList().iterator(); iterator
						.hasNext();) {
				...
				// interval
				if ((pagination.getPagesTotal() - pagination.getStart() > (pagination
						.getStep() / 2))
						&& pagination.getPagesTotal() > pagination.getStep()) {
					buffer.append("<li>...</li>");
					buffer.append("<li class='numero_off'>");
					makeListLink(buffer, pagination.getPagesTotal(), pagination
							.getPagesTotal()
							+ "");
					buffer.append("</li>");
				}
			...
			if (showDescription) {
				...
				buffer.append(((pagination.getStartPage() - 1) * pagination
						.getSize()) + 1);
				buffer.append("</strong>");
				buffer.append(" a ");
				buffer.append("<strong>");
				if (((pagination.getStartPage()) * pagination.getSize()) + 1 <= pagination
						.getTotal()) {
					buffer.append(pagination.getStartPage()
							* pagination.getSize());
				} else {
		...
		try {
			...
			LOGGER.error(
					"PaginationTag, doStartTag: Erro ao criar a paginação", e);
		}
	...
	public Collection<Integer> getPageList() {
		...
		if (pagination.getStartPage() < pagination.getStep() / 2) {
			cont = var;
		} else if (pagination.getPagesTotal() - pagination.getStartPage() < pagination
				.getStep() / 2) {
			cont = pagination.getStep()
					- (pagination.getPagesTotal() - pagination.getStartPage())
					- 1;
		...
		while (var2 <= pagination.getPagesTotal()
				&& cont2 < pagination.getStep() - 1) {
